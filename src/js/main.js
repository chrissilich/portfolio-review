...
let showHideStudents = () => {
	;[...allStudents, ...allStudentSpacers].forEach((item) => {
		if (
			(item.dataset.program === pickedProgram || 'all' === pickedProgram) &&
			(item.dataset.cohort === pickedCohort || 'all' === pickedCohort)
		) {
			// show!
...
allStudents.forEach((student) => {
	...
	if (isTouchDevice()) {
		...
		student.querySelector('.student-content').addEventListener('mouseenter', over)
		student.querySelector('.student-content').addEventListener('mouseleave', out)
	}
})
